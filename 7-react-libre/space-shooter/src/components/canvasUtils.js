 ...
 */
export const drawBackground = (context, backgroundImg, canvasWidth, canvasHeight) => {
 ...
 */
export const drawTeleportEffects = (context, teleportEffects, players, teleportEffectImg) => {
    teleportEffects.forEach(effect => {
        ...
        if (elapsed < teleportEffectDuration && player) {
            ...
            context.drawImage(teleportEffectImg, -halfEffectWidth, -halfTeleportDistance, teleportRenderWidth, teleportEffectDistance);
            context.restore();
        }
    });
};
 ...
 * @param {CanvasRenderingContext2D} context - The canvas rendering context.
 * @param {object} players - The players data.
 * @param {HTMLImageElement} redShipImg - The red ship image.
 * @param {HTMLImageElement} blueShipImg - The blue ship image.
 * @param {number} canvasWidth - The width of the canvas.
 * @param {number} canvasHeight - The height of the canvas.
 */
export const drawPlayers = (context, players, redShipImg, blueShipImg, canvasWidth, canvasHeight) => {
    Object.values(players).forEach(player => {
        player.x = (player.x + canvasWidth) % canvasWidth;
        player.y = (player.y + canvasHeight) % canvasHeight;

        context.save();
        context.translate(player.x + halfPlayerWidth, player.y + halfPlayerHeight);
        context.rotate(player.angle + halfPi);
 ...
 */
export const drawProjectiles = (context, projectiles, players, laserRedImg, laserBlueImg) => {
    projectiles.forEach(projectile => {
        ...
        context.drawImage(projectileImg, -projectileRadius, -projectileRadius, projectileRadius * 2, projectileRadius * 2);
        context.restore();
    });
};
...
export const drawRespawnCountdowns = (context, respawnCountdown, players) => {
    Object.keys(respawnCountdown).forEach(playerId => {
      ...
      if (player) {
        ...
        context.fillText(respawnCountdown[playerId], player.x + halfPlayerWidth, player.y + halfPlayerHeight);
        context.restore();
      }
    });
  };
